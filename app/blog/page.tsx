    ...
    <div className="min-h-screen bg-[#f0ede8] dark:bg-[#0d0d0d] text-[#111] dark:text-[#f0ede8] transition-colors duration-300">
      ...
      <header className="sticky top-0 z-[100] flex items-center justify-between py-5 px-12 bg-[#f0ede8]/90 dark:bg-[#0d0d0d]/80 backdrop-blur-xl border-b border-black/10 dark:border-white/10 tablet:px-6">
        ...
        <nav className="flex items-center gap-6">
          <Link href="/" className="text-[14px] font-medium text-[#555] dark:text-[#999] hover:text-[#111] dark:hover:text-white transition-colors">← Portfolio</Link>
      ...
      <section className="px-12 pt-20 pb-12 max-w-[900px] mx-auto tablet:px-6">
        <motion.div initial="hidden" animate="show" variants={stagger}>
          <motion.p variants={fadeUp} className="text-[12px] uppercase tracking-[3px] text-accent font-semibold mb-4">Writing</motion.p>
          <motion.h1 variants={fadeUp} className="font-black leading-[0.95] uppercase mb-6" style={{ fontSize: "clamp(48px, 8vw, 100px)" }}>Blog</motion.h1>
      ...
      <div className="h-px bg-black/10 dark:bg-white/10 max-w-[900px] mx-auto" />
      ...
      <section className="px-12 py-16 max-w-[900px] mx-auto tablet:px-6">
        ...
        {error && (
          <div className="py-16 text-center">
            <p className="text-[#999] text-[15px]">Could not load posts.</p>
                ...
                <div className="flex items-start justify-between gap-6 tablet:flex-col tablet:gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span className="text-[13px] text-[#777] dark:text-[#888]">{readingTime(post.excerpt + " " + post.content)} min read</span>
                    </div>
                    <h2 className="text-[22px] font-bold text-[#111] dark:text-white group-hover:text-accent transition-colors mb-2 leading-snug">{post.title}</h2>
                    {post.excerpt && <p className="text-[15px] text-[#555] dark:text-[#999] leading-[1.7] line-clamp-2">{post.excerpt}</p>}
                  </div>
                  {post.coverImage && (
                    <img src={post.coverImage} alt="" className="w-24 h-24 rounded-xl object-cover flex-shrink-0 tablet:w-full tablet:h-48" />
                  )}
                </div>
                <Link href={`/blog/${post.slug}`} className="inline-flex items-center gap-2 mt-5 text-[14px] font-semibold text-accent hover:gap-3 transition-all">
                  Read article →
                </Link>
              </motion.article>
            ))}
