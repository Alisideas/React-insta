...
const fadeUp    = { hidden: { opacity: 0, y: 50 },  show: { opacity: 1, y: 0,  transition: { duration: 0.7, ease: EASE } } };
const fadeIn    = { hidden: { opacity: 0 },           show: { opacity: 1,        transition: { duration: 0.6 } } };
const slideLeft = { hidden: { opacity: 0, x: -60 }, show: { opacity: 1, x: 0,  transition: { duration: 0.7, ease: EASE } } };
const slideRight= { hidden: { opacity: 0, x: 60 },  show: { opacity: 1, x: 0,  transition: { duration: 0.7, ease: EASE } } };
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit={{ opacity: 0, scale: 0.93, y: 24 }} transition={{ duration: 0.35, ease: EASE }}
      ...
      >
        <div className="flex items-center justify-between px-8 pt-8 pb-6 border-b border-black/10 dark:border-white/10">
          <div>
            <h2 className="text-[24px] font-extrabold text-[#111] dark:text-white">Get In <span className="text-accent">Touch</span></h2>
            <p className="text-[#666] dark:text-[#999] text-[14px] mt-1">I&apos;ll reply within 24 hours.</p>
          </div>
          <button onClick={onClose} className="w-9 h-9 rounded-xl bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 flex items-center justify-center text-[#666] dark:text-[#999] hover:text-[#111] dark:hover:text-white hover:bg-black/10 dark:hover:bg-white/10 transition-all text-lg leading-none">✕</button>
        </div>
        <div className="px-8 py-6">
          {status === "success" ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, ease: EASE }}
              ...
              <button onClick={onClose} className="mt-2 px-6 py-2 rounded-xl bg-black/5 dark:bg-white/5 border border-black/15 dark:border-white/15 text-[#666] dark:text-[#999] hover:text-[#111] dark:hover:text-white transition-all text-[14px]">Close</button>
            </motion.div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              {status === "error" && (
                <p className="text-red-500 text-[13px] bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3">
                  Something went wrong. Please try again or email <a href="mailto:alirezakbarim@gmail.com" className="underline">alirezakbarim@gmail.com</a>.
                </p>
              )}
...
export default function HomePage() {
  ...
  const techStack = [
    ...
    "UI/UX Design","TypeScript","Tailwind CSS","PHP","Figma","PostgreSQL","Firebase",
  ];
  ...
  const experience = [
    { role: "Senior Web Designer",   company: "Mishov Markets",  period: "Sep 2025 – Feb 2026", location: "Istanbul, Türkiye", description: "Designed and delivered high-quality web interfaces for a fast-growing markets platform, focusing on visual consistency and user experience.", tags: ["UI/UX","Web Design","React"] },
    { role: "Full-stack Developer",  company: "NiaTech",         period: "May 2024 – Nov 2024", location: "Istanbul, Türkiye", description: "Developed modern, high-performance web applications using Next.js. Designed and built user interfaces, developed backend APIs, worked with databases, and ensured application performance and security.", tags: ["Next.js","React","Node.js","APIs"] },
    { role: "Frontend Developer",    company: "Dextern",         period: "Jun 2022 – Jan 2024", location: "Ireland",           description: "Developed modern, interactive web applications using React.js and cross-platform mobile apps with Flutter. Also used WordPress for dynamic websites, implementing responsive UIs and optimizing performance.", tags: ["React.js","Flutter","WordPress"] },
    { role: "Web Developer",         company: "Zebrit",          period: "Jun 2022 – Nov 2023", location: "Ireland",           description: "Built and customized WordPress websites for various clients. Developed custom themes and plugins, optimized performance, ensured responsive design, and managed content with Elementor.", tags: ["WordPress","Elementor","PHP"] },
    { role: "Web Developer",         company: "49 Tech Hub",     period: "Apr 2021 – Jun 2022", location: "Istanbul, Turkey", description: "Created WordPress-based websites with custom themes and plugins. Optimized website performance and provided ongoing technical support and regular updates for client projects.", tags: ["WordPress","Elementor","HTML/CSS"] },
  ];
    ...
    <div className="min-h-screen flex flex-col bg-[#f0ede8] dark:bg-[#0d0d0d] text-[#111] dark:text-[#f0ede8] transition-colors duration-300 overflow-x-hidden">
      ...
      >
        ...
        >
          ...
          <motion.div variants={fadeIn}>
            <Link href="/blog" className="text-[#555] dark:text-[#999] text-[14px] font-medium hover:text-[#111] dark:hover:text-white transition-colors">Blog</Link>
          </motion.div>
        </motion.nav>
      ...
      <section className="px-12 pt-20 pb-10 max-w-[1200px] mx-auto w-full tablet:px-6" id="hero">
        <div className="flex items-start justify-between gap-12 tablet:flex-col">
          <motion.div className="flex-1" initial="hidden" animate="show" variants={stagger}>
            <motion.p variants={fadeIn} className="text-[12px] font-semibold uppercase tracking-[3px] text-accent mb-5">Available for work</motion.p>
            <motion.h1 variants={fadeUp} className="font-black leading-[0.9] uppercase text-[#111] dark:text-[#f0ede8]" style={{ fontSize: "clamp(54px, 10.5vw, 148px)" }}>Full-stack</motion.h1>
            <motion.h1 variants={fadeUp} className="font-black leading-[0.9] uppercase text-[#111] dark:text-[#f0ede8]" style={{ fontSize: "clamp(54px, 10.5vw, 148px)" }}>Developer</motion.h1>
            <motion.div variants={fadeUp} className="mt-7 flex items-center gap-3">
              <div className="h-px w-16 bg-black/25 dark:bg-white/25" />
              <span className="text-[13px] uppercase tracking-[2px] text-[#777] dark:text-[#888]">Alireza Akbari</span>
            </motion.div>
            <motion.div variants={staggerFast} className="hidden tablet:grid grid-cols-3 gap-6 mt-12 pt-8 border-t border-black/10 dark:border-white/10">
              {[{ num: "5+", label: "Years Exp." },{ num: "5", label: "Companies" },{ num: "2", label: "Degrees" }].map(({ num, label }) => (
          ...
          <motion.div className="flex-shrink-0 w-[340px] tablet:w-full tablet:max-w-[480px] mobile:max-w-full" variants={slideRight} initial="hidden" animate="show">
            ...
            <p className="mt-10 text-[15px] text-[#555] dark:text-[#999] leading-[1.8]">
              Passionate about building visually appealing and user-friendly products. Experienced in React.js, Next.js, Flutter, and WordPress — currently based in Istanbul.
            </p>
            <div className="mt-6 flex gap-3">
              <a href="#experience" className="flex-1 text-center px-4 py-3 rounded-full text-[14px] font-semibold bg-[#111] dark:bg-white text-white dark:text-[#111] hover:bg-accent dark:hover:bg-accent dark:hover:text-white transition-colors">View Work</a>
              <button onClick={() => setContactOpen(true)} className="flex-1 px-4 py-3 rounded-full text-[14px] font-semibold border border-black/20 dark:border-white/20 text-[#555] dark:text-[#999] hover:border-accent hover:text-accent transition-colors cursor-pointer">Contact Me →</button>
            </div>
          </motion.div>
        </div>

        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65, duration: 0.6, ease: EASE }}
          className="mt-20 pt-8 border-t border-black/10 dark:border-white/10 grid grid-cols-4 tablet:hidden gap-6">
          {[{ num: "5+", label: "Years Experience" },{ num: "5", label: "Companies Worked" },{ num: "2", label: "Degrees" },{ num: "∞", label: "Curiosity" }].map(({ num, label }) => (
      ...
      <div className="bg-accent text-white py-4 mt-12 overflow-hidden">
        <div className="flex animate-marquee whitespace-nowrap" style={{ width: "max-content" }}>
          {[...techStack, ...techStack].map((tech, i) => (
            <span key={i} className="mx-8 text-[13px] font-semibold uppercase tracking-[2px] flex-shrink-0">◆ {tech}</span>
          ))}
        </div>
      </div>
      ...
      <section className="px-12 py-24 max-w-[1200px] mx-auto w-full tablet:px-6" id="about">
        <motion.div initial="hidden" whileInView="show" viewport={VP} variants={stagger}
          ...
          <motion.div variants={slideLeft}>
            ...
            <h2 className="text-[42px] font-black leading-[1.1] mb-6 mobile:text-[30px]">Building things<br />that <span className="text-accent">matter</span></h2>
            <p className="text-[16px] text-[#555] dark:text-[#999] leading-[1.8] mb-4">
              I&apos;m <strong className="text-[#111] dark:text-white">Alireza</strong>, a passionate web developer and UI/UX designer with a strong background in HTML, CSS, JavaScript, Flutter, and React.js. I love creating beautiful, functional products that people enjoy using.
            </p>
            <p className="text-[16px] text-[#555] dark:text-[#999] leading-[1.8] mb-8">
              I&apos;ve worked on e-commerce websites, marketing sites, and mobile applications. Always learning, always building — currently based in <strong className="text-[#111] dark:text-white">Istanbul</strong>.
            </p>
            <button onClick={() => setContactOpen(true)} className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#111] dark:bg-white text-white dark:text-[#111] text-[14px] font-semibold hover:bg-accent dark:hover:bg-accent dark:hover:text-white transition-colors cursor-pointer">
              Let&apos;s work together →
            </button>
          </motion.div>
          <motion.div variants={slideRight} className="flex flex-col gap-0">
            {[
              ...
              <motion.div key={i} variants={fadeUp} className="flex items-start gap-6 py-8 border-b border-black/10 dark:border-white/10 first:pt-0 last:border-0 last:pb-0">
        ...
        <motion.div className="px-12 py-24 max-w-[1200px] mx-auto w-full tablet:px-6" initial="hidden" whileInView="show" viewport={VP} variants={stagger}>
          <div className="grid grid-cols-2 gap-20 items-start tablet:grid-cols-1 tablet:gap-12">
            <motion.div variants={slideLeft}>
              ...
              <h2 className="text-[42px] font-black leading-[1.1] mb-6 mobile:text-[30px]">Services &<br /><span className="text-accent">Skills</span></h2>
              <p className="text-[16px] text-[#555] dark:text-[#999] leading-[1.8] mb-10">From concept to production — I cover the full stack with a design-first mindset.</p>
            ...
            <motion.div variants={slideRight} className="grid grid-cols-2 gap-4 mobile:grid-cols-1">
              {services.map((svc, i) => (
                <motion.div key={i} variants={fadeUp} whileHover={{ y: -4, transition: { duration: 0.2 } }}
                  className={`p-7 rounded-2xl flex flex-col gap-4 border transition-all ${svc.highlighted ? "bg-accent border-accent text-white" : "bg-white dark:bg-white/[0.04] border-black/10 dark:border-white/10 hover:border-accent/50"}`}>
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[16px] ${svc.highlighted ? "bg-white/20" : "bg-accent/10"}`}>
                    {["⚡","✦","📱","🌐"][i]}
                  </div>
                  <h3 className={`text-[16px] font-bold leading-snug ${svc.highlighted ? "text-white" : "text-[#111] dark:text-white"}`}>{svc.title}</h3>
                  <p className={`text-[14px] leading-[1.7] ${svc.highlighted ? "text-white/80" : "text-[#555] dark:text-[#999]"}`}>{svc.desc}</p>
                </motion.div>
              ))}
            </motion.div>
          </div>
        ...
        <motion.div className="px-12 py-24 max-w-[1200px] mx-auto w-full tablet:px-6" initial="hidden" whileInView="show" viewport={VP} variants={stagger}>
          ...
          <div className="flex flex-col">
            {experience.map((job, i) => (
              <motion.div key={i} variants={fadeUp} whileHover={{ x: 4, transition: { duration: 0.15 } }}
                ...
                <div className="flex-1">
                  <h3 className="text-[18px] font-bold text-[#111] dark:text-white group-hover:text-accent transition-colors">{job.role}</h3>
          ...
          <div className="flex flex-col">
            {education.map((edu, i) => (
              <motion.div key={i} variants={fadeUp} whileHover={{ x: 4, transition: { duration: 0.15 } }}
                ...
                <div className="flex-shrink-0 w-[150px] tablet:w-auto">
                  <span className="text-[13px] text-[#777] dark:text-[#888]">{edu.period.split(" – ")[0]}</span>
                  {edu.ongoing && <span className="block text-[11px] font-bold text-accent uppercase tracking-[0.5px] mt-1">In Progress</span>}
                </div>
                <div className="flex-1">
                  <h3 className="text-[20px] font-bold text-[#111] dark:text-white group-hover:text-accent transition-colors leading-[1.3]">{edu.degree}</h3>
      ...
      <footer className="bg-[#0d0d0d] text-[#f0ede8]" id="contact">
        <div className="px-12 pt-24 pb-12 max-w-[1200px] mx-auto w-full tablet:px-6">
          <motion.div initial="hidden" whileInView="show" viewport={VP} variants={stagger}>
            <motion.p variants={fadeIn} className="text-[12px] uppercase tracking-[3px] text-accent font-semibold mb-6">Get In Touch</motion.p>
            <motion.h2 variants={fadeUp} className="font-black leading-[0.93] uppercase text-[#f0ede8] mb-12" style={{ fontSize: "clamp(44px, 8.5vw, 124px)" }}>
              Let&apos;s Connect<br />There
            </motion.h2>
            <motion.div variants={fadeUp} className="flex gap-4 flex-wrap mb-20">
              <button onClick={() => setContactOpen(true)} className="px-8 py-4 rounded-full bg-accent text-white text-[15px] font-semibold hover:bg-accent-dark transition-colors shadow-[0_0_30px_rgba(224,53,0,0.4)] cursor-pointer">Send a Message</button>
              <a href="mailto:alirezakbarim@gmail.com" className="px-8 py-4 rounded-full border border-white/20 text-[#999] text-[15px] font-semibold hover:border-accent hover:text-accent transition-colors">alirezakbarim@gmail.com</a>
            </motion.div>
